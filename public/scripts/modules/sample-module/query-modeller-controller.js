	...
	for ($scope.tb=0;$scope.tb<getQueryForm.ripos.options.length;$scope.tb++){
		if (getQueryForm.ripos.options[$scope.tb].selected){
			...
			for($scope.attrs in $scope.dbObject[dataBase][getQueryForm.ripos.options[$scope.tb].text]){
				...
				for(var i=0;i<$scope.dbObject[dataBase][getQueryForm.ripos.options[$scope.tb].text].length;i++)
				{
					$scope.TableColumns[i]=$scope.attrs;
				}
...
$scope.generateQuery = function(){
	...
	$scope.queryObject={
		...
		"query":function(){
				return "SELECT row_number() over() as rownum,"+$scope.queryObject.attribute+" FROM "+$scope.queryObject.table+" WHERE "+whereClauseString();
			}
	};
...
//sub string in sql from multi selection box
	...
	$scope.QueryObject = $scope.queryObject.query();
}
